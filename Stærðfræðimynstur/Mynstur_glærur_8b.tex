...
% \begin{frame}{Dæmi}
% Finnum lausn á rakningarvenslunum $a_n = 3a_{n-1} + 2n$ með $a_1 = 3$. \pause
% \vspace{1cm}
% Leysum fyrst tilsvarandi óhliðruðu rakningarvenslin, fáum $a_n^{(h)} = \alpha 3^n$, þar sem $\alpha$ er fasti. Giskum á að sérlausn sé á sniðinu $p_n = cn + d$. Út frá $a_n = 3a_{n-1} + 2n$ fáum við $3(c(n-1)+d)+2n$ $= cn + d$, sem er jafngilt $(2+2c)n + (2d-3c) = 0$, svo $2+2c = 0$ og $2d-3c = 0$, svo $c = -1$ og $d = -\frac{3}{2}$.
% \end{frame}
% 
% \begin{frame}{Dæmi}
% Fáum að 
% 
% Nevermind, þetta er allt of langt fyrir þetta show.
% \end{frame}
